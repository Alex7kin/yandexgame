/* ============================================================
   ...
   Packaged as a GameHost module: it mounts into the shared #stage, runs until
   unmounted (Back to menu), and cleans up its loop, listeners and audio.
   ============================================================ */
(function () {
  "use strict";

  // ---------- Tunable design constants (shared, immutable; units relative to H) ----------
  const GROUND_FRAC      = 0.80;  // road surface (wheel contact line)
  const COURIER_FRAC     = 0.22;  // courier display height (before ACTOR_SCALE)

  // Portrait-locked (~0.5 aspect) on every device, so one scale fits all frames.
  const ACTOR_SCALE      = 0.55;

  // Jump physics (per-second values multiplied by H so the feel is resolution-independent)
  const JUMP_V0          = 1.30;
  const GRAVITY          = 5.0;
  const GRAVITY_HOLD     = 2.1;
  const JUMP_CUT         = 0.80;
  const MAX_HOLD         = 0.26;
  ...
  const MAX_SPEED_BONUS  = 1.15;

  // Railings — elevated ledges you jump ONTO and ride; ram the front face and you crash.
  const RAIL_MIN_W       = 0.55;
  const RAIL_MAX_W       = 1.20;
  const RAIL_LEVELS      = [0.09, 0.155, 0.22];
  const RAIL_GAP         = 0.55;
  const RAIL_CRASH_TOL   = 0.02;

  // Animation
  const PEDAL_CADENCE    = 7.5;

  // Drone-crash cutscene (one-off spectacle)
  const DRONE_AT         = 11000;
  const DRONE_FLY        = 1.4;
  const EXPLODE_DUR      = 1.1;
  const SETTLE_DUR       = 0.6;

  const READY = 0, PLAYING = 1, GAMEOVER = 2, CUTSCENE = 3;

  // ---------- Audio assets: fetched once for the page, decoded per audio context ----------
  let explosionBytes = null, crashBytes = null;
  fetch("sounds/explosion.mp3")
    .then((r) => (r.ok ? r.arrayBuffer() : Promise.reject(r.status)))
    .then((ab) => { explosionBytes = ab; }).catch(() => {});
  fetch("sounds/crash.mp3")
    .then((r) => (r.ok ? r.arrayBuffer() : Promise.reject(r.status)))
    .then((ab) => { crashBytes = ab; }).catch(() => {});

  // ---------- Portrait lock (shared media query) ----------
  // Touch devices are portrait-only; in landscape we freeze and CSS shows #rotate.
  const portraitLock = window.matchMedia("(orientation: landscape) and (pointer: coarse)");
  const PORTRAIT_RATIO = 0.5;   // desktop renders the game in this W:H portrait frame

  function create() {
    // ----- handles wired on mount -----
    let host = null, canvas = null, ctx = null, stage = null;
    let elScore, elBest, startScreen, gameOverScreen, elFinalScore, elFinalBest;

    // ----- per-run state -----
    let W = 0, H = 0, DPR = 1, skyGrad = null, roadGrad = null;
    let groundY = 0, courierX = 0, courierH = 0, courierW = 0;
    let actorScale = ACTOR_SCALE;
    let speed = 0, distance = 0, scoreFloat = 0, score = 0, best = 0;
    let nextPointAt = 100, restartAllowedAt = 0;
    let state = READY;
    const courier = { y: 0, vy: 0, grounded: true, holding: false, jumpTime: 0 };
    let coyote = 0, pedalPhase = 0;
    let rails = [], spawnTimer = 0;
    let cloudX = 0, bldX = 0, roadX = 0;
    let cutTime = 0, cutExploded = false, droneDone = false, cutParticles = [];
    let last = 0, raf = null;

    // ----- audio (per instance / context) -----
    let audioCtx = null, explosionBuf = null, crashBuf = null, droneNodes = null;

    // ----- listener bookkeeping so unmount can detach everything -----
    const listeners = [];
    ...
    function resize() {
      ...
      if (window.matchMedia("(pointer: coarse)").matches) {
        W = winW; H = winH;                                   // touch: fill (portrait-locked)
      } else if (winW / winH > PORTRAIT_RATIO) {
        H = winH; W = Math.round(winH * PORTRAIT_RATIO);      // desktop: portrait frame
      } else {
    ...
    function playExplosion() {
      if (!host || host.muted || !audioCtx || !explosionBuf) return;
      try {
    ...
    function playCrash() {
      if (!host || host.muted || !audioCtx || !crashBuf) return;
      try {
        const src = audioCtx.createBufferSource();
        src.buffer = crashBuf;
        const g = audioCtx.createGain();
    ...
    // Sustained quadcopter buzz for the cutscene; held until stopDroneSound() at impact.
    function startDroneSound() {
      ...
      try {
        ...
        master.gain.exponentialRampToValueAtTime(0.05, t + 0.2);
        ...
        lfo.frequency.linearRampToValueAtTime(44, end);
    ...
    function stopDroneSound() {
      ...
      try {
        ...
        n.master.gain.exponentialRampToValueAtTime(0.0001, t + 0.08);
        n.a.stop(t + 0.1); n.b.stop(t + 0.1); n.lfo.stop(t + 0.1);
      } catch (_) {}
    }

    // ============================================================
    //  Input — keyboard, mouse, touch & pen
    // ============================================================
    function pressJump() {
    ...
    function onPointerDown(e) {
      if (e.target.closest && e.target.closest(".btn")) return; // let the mute / back buttons work
      e.preventDefault();
      pressJump();
    }
    function onPointerUp() { releaseJump(); }
    ...
    function gameOver() {
      ...
      updateHUD();
      host.submitScore(score); // record on Telegram's leaderboard if this is the ranked game
    ...
    function update(dt) {
      ...
      // HALF on (one-way). Being half-on is also what keeps you "grounded", so you
      // can still jump right at the edge.
      let surface = groundY;
      for (const r of rails) {
        const onTop = Math.min(footR, r.x + r.w) - Math.max(footL, r.x);
        if (onTop >= half && r.top < surface && prevFeet <= r.top + 1) surface = r.top;
      }
      if (courier.vy >= 0 && courier.y >= surface) {
        courier.y = surface;
        courier.vy = 0;
        courier.grounded = true;
        courier.holding = false;
      ...
      // Coyote time: keep "can jump" alive for a beat after rolling off an edge.
      coyote = courier.grounded ? COYOTE_TIME : Math.max(0, coyote - dt);

      // Crash: hit a railing's FRONT (left) face with the wheels below its top.
    ...
    function drawRail(r) {
      ...
      ctx.fillRect(x, top, w, Math.max(1, barH * 0.32));
      ctx.fillStyle = "rgba(0,0,0,0.18)";
      ctx.fillRect(x, top + barH, w, Math.max(1, barH * 0.25));
    }
    ...
    function drawCourier() {
      ...
      ctx.beginPath();
      ctx.moveTo(bb.x, bb.y);   ctx.lineTo(rearW.x, rearW.y);
      ctx.moveTo(seat.x, seat.y); ctx.lineTo(rearW.x, rearW.y);
      ctx.moveTo(seat.x, seat.y); ctx.lineTo(bb.x, bb.y);
      ctx.moveTo(bb.x, bb.y);   ctx.lineTo(head.x, head.y);
      ctx.moveTo(seat.x, seat.y); ctx.lineTo(head.x, head.y);
      ctx.moveTo(head.x, head.y); ctx.lineTo(frontW.x, frontW.y);
      ctx.moveTo(head.x, head.y); ctx.lineTo(bar.x, bar.y);
      ctx.stroke();
      ...
      ctx.restore();
      ctx.strokeStyle = "rgba(0,0,0,0.32)"; ctx.lineWidth = 0.012 * u;
      ...
      ctx.beginPath(); ctx.arc(faceC.x + faceR * 0.95, faceC.y + faceR * 0.18, faceR * 0.17, 0, Math.PI * 2); ctx.fill();
    ...
    function updateCutscene(dt) {
      ...
      if (cutTime >= DRONE_FLY + EXPLODE_DUR + SETTLE_DUR) state = PLAYING;
    }

    function drawCutBuilding(g, fade) {
      ctx.globalAlpha = fade;
      ctx.fillStyle = "#8a79b6";
    ...
    function drawDrone(t, g) {
      ...
      const ang = Math.sin(t * 20) * (1 - p) * 0.28 + ease * 0.9;
      const s = 0.10 * H;
      ...
      ctx.rotate(ang);
      ctx.lineCap = "round";
      ctx.strokeStyle = "#3a3a42"; ctx.lineWidth = s * 0.1;
      ctx.beginPath();
      ...
      ctx.strokeStyle = "#33333a"; ctx.lineWidth = s * 0.03;
      ctx.beginPath();
    ...
    function mount(h) {
      ...
      speed = 0; distance = 0; scoreFloat = 0; score = 0; nextPointAt = 100;
      rails = []; spawnTimer = 0;
      cloudX = 0; bldX = 0; roadX = 0; pedalPhase = 0;
      courier.y = 0; courier.vy = 0; courier.grounded = true; courier.holding = false; courier.jumpTime = 0;
      coyote = 0; droneDone = false; cutParticles = []; last = 0;

      // Silence the drone instantly if the player mutes mid-cutscene.
      host.onMuteToggle = (m) => { if (m) stopDroneSound(); };
    ...
    function unmount() {
      ...
      explosionBuf = null; crashBuf = null;   // were decoded against the now-closed context
    ...
    return {
      id: "bikecourier",
      title: "Bike Courier",
      tagline: "Ride & jump the railings",
      emoji: "🚴",
      accent: "#ffcc00",
      start: {
        title: "Bike&nbsp;Courier",
        lead: "Jump onto the railings and ride!",
        hint: "<b>Space</b> / <b>Tap</b> to jump",
        sub: "Hold longer to jump higher",
      },
      over: { title: "Crash!" },
      mount,
      unmount,
